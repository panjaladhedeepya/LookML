...
###############################################################################################
  ...
  dimension: ibe_spending_value {
    type: number
    sql:  CASE WHEN LOWER(${ibe_spending.value}) LIKE '%not confirmed%' THEN 0
                WHEN LOWER(${ibe_spending.value}) = 'missing' THEN 0
                WHEN ${ibe_spending.value} = "0" THEN 0
                ELSE 1 END ;;
  }

  measure: spending_target_sum {
    type: number
    sql:SUM(CASE WHEN ${target_flg} = {% parameter ib_data_portrait.target_select %} THEN ${ibe_spending_value} ELSE 0 END);;
  }

  measure: spending_target_count {
    # sql_distinct_key:${ibe_interests} ;;
    type: number
    sql:SUM(CASE WHEN ${target_flg} = {% parameter ib_data_portrait.target_select %} THEN 1 ELSE 0 END);;
  }

  measure: spending_reference_sum {
    type: number
    sql:SUM(CASE WHEN ${target_flg} = {% parameter ib_data_portrait.reference_select %} THEN ${ibe_spending_value} ELSE 0 END);;
  }

  measure: spending_reference_count {
    type: number
    sql:SUM(CASE WHEN ${target_flg} = {% parameter ib_data_portrait.reference_select %} THEN 1 ELSE 0 END);;
  }

  measure: spending_percent_of_total_target {
    type: number
    value_format_name: percent_1
    sql: (${spending_target_sum}/NULLIF(${spending_target_count},0));;
  }

  measure: spending_percent_of_total_reference {
    type: number
    value_format_name: percent_1
    sql: (${spending_reference_sum}/NULLIF(${spending_reference_count},0));;
  }

  measure: spending_index {
    type: number
    value_format_name: decimal_1
    sql: (${spending_percent_of_total_target}/NULLIF(${spending_percent_of_total_reference},0)) * 100;;
  }

  # measure: spending_index_html {
  ...
  measure: spending_index_html {
    ...
    html:
      ...
      {% elsif spending_index._value >= 180 %}
      ...
      {% elsif spending_index._value >= 120 %}
      ...
      {% elsif spending_index._value >= 80 %}
      ...
      {% elsif spending_index._value >= 50 %}
      ...
      {% elsif spending_index._value >= 20 %}
